...
def cornersHeuristic(state, problem):
    ...
    for x in range(len(corners)):
            if state[1][x]:
                temp = util.manhattanDistance(corners[x], state[0])
                if temp < heur:
                    heur = temp
                    heurLocation = x
    #print("closest goal is: ", heur)
    for x in range(len(corners)):
        if state[1][x] and x != heurLocation:
            temp = util.manhattanDistance(corners[x], corners[heurLocation])
            if temp < temp1:
                temp1 = temp
                tempLocation = x
    if temp1 == 10000:
        temp1 = 0
    #print("next goal is: ", temp1)
    heur += temp1
